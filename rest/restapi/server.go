...
// NewServer creates a new api p2p server but does not configure it
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/p2p.sock"`
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(2)
		s.Logf("Serving p2p at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving p2p at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(2)
		s.Logf("Serving p2p at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving p2p at http://%s", l.Addr())
		}(s.httpServerL)
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(2)
		s.Logf("Serving p2p at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving p2p at https://%s", l.Addr())
